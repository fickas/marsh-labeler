...
## Exporting GT back (after labeling)

Verdicts live in the Railway DB, so run `export_gt` locally with the **Railway**
env loaded and the canonical `gt_labeled.gpkg` brought in first (see RUNBOOK's
"never export onto a missing GT" rule and the `--init` guard):
...
It reads verdicts from Railway, reconciles into the local gpkg, which you then
sync back to Drive.
