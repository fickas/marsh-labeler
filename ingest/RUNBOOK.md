...
| `selection_params_<date>.json`            | `selection_params.json`      | "Why this tile?" panel (optional) |
...
- **The "Why this tile?" panel is optional.** If you bring in `selection_params.json`,
  add `selection_params_path: data/synthetic/selection_params.json` to the YAML; ingest
  stores it on the flight and the labeler sees the explanation. Omit it and the panel
  just doesn't render -- nothing else changes.
